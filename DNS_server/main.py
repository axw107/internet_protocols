...
class DNS:
    ...
    def make_response(self, data: bytes) -> bytes:
        """
        Обрабатывает входящий DNS-запрос и формирует ответ
    ...
    def build_response(self, data: bytes) -> bytes:
        """
        Строит ответ на основе входящих данных.
        :param data:
        :return:
        """
        ...
        body = (
            b''.join([self.record_to_bytes(record_type, record['ttl'], record['value']) for record in records]))
    ...
    def get_records(self, data) -> tuple[list, str, str]:
        """
        Извлекает записи DNS из кэша или запрашивает их у доверенного сервера.
        :param data:
        :return: Кортеж, содержащий список записей, тип записи и домен
        """
    ...
    def build_response_flags(self, flags: bytes) -> bytes:
        """
        Строит флаги ответа на основе входящих флагов.
        :param flags: флаги запроса в формате байтов.
        :return: Флаги ответа в формате байтов
        """
        first_byte = flags[:1]
        flags = Flags(
    ...
    def build_question(domain: str, record_type: str) -> bytes:
        """
        Строит вопрос для DNS-запроса.
        :param domain: Домен, для которого формируется вопрос.
        :param record_type: Тип записи (например, 'a' или 'ns').
        :return: Вопрос в формате байтов.
        """
        question = b''
    ...
    def record_to_bytes(record_type: str, ttl: int, value: str) -> bytes:
        """
        Преобразует запись в формат байтов.
        :param record_type: Тип записи
        :param ttl: Время жизни записи
        :param value:  Значение записи
        :return: Запись в формате байтов
        """
        record = b'\xc0\x0c'
    ...
    def make_info_from_response(self, data: bytes, domain: str, qtype: str) -> dict:
        """
        Создает информацию о записи из ответа.
        :param data: Данные ответа в формате байтов
        :param domain:  Домен, для которого получен отвеn
        :param qtype: Тип запроса
        :return: Словарь с информацией о записи
        """
    ...
    def get_records_from_answer(self, answer: bytes, count: int) -> dict:
        """
        Извлекает записи из ответа.
        :param answer:  Данные ответа в формате байтов
    ...
    def find_data(self, domain: str, qtype: str) -> dict:
        """
        Находит данные для домена, отправляя запрос к доверенному серверу.
        :param domain: Домен, для которого нужно найти данные
        :param qtype: Тип записи
    ...
    def get_info(self, domain: str, qtype: str) -> dict:
        """
        Получает информацию о домене из кэша или запрашивает у доверенного сервера.
        :param domain: Домен, для которого нужно получить информацию
        :param qtype: Тип записи
